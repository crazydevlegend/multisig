...
solana airdrop 100 $ALPHA
solana airdrop 100 $BETA
solana airdrop 100 $DELTA

npm run build:program
PROGRAM=$(npm run start deploy-async -- --authority keys/alpha.json | tee /dev/tty | rg 'Program deployed to account' | cut -d ' ' -f5)
